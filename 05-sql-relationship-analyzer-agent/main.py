...
with MCPServerAdapter(
    ...
    database_analyst_agent = Agent(
        ...
        goal=f"""Extract all table relationships (foreign keys) from {DATABASE_NAME}.
    ...
    validator_agent = Agent(
        ...
        goal=f"""Validate the extracted relationships (foreign keys) for {DATABASE_NAME}.
        ...
        WORKFLOW:
        1. Always attempt to use MSSQL MCP tools first ({tool_list_str}).
        2. If they don’t return full details, fallback to sys.foreign_keys and sys.foreign_key_columns queries through MCP.
        3. Never invent or assume missing details.
    ...
    database_analysis_task = Task(
        description=f"""Query {DATABASE_NAME} and return all relationships (foreign keys).
    ...
    validation_task = Task(
        description=f"""Validate extracted foreign key relationships from {DATABASE_NAME}.
        ...
        {{
            ...
            "issues": [
                {{
                "type": "missing_fk|invalid_fk|other",
                "name": "FK_Name",
